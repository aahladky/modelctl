...
Check the responses against `modelctl_capabilities.py`: schema handling must fail closed for unsupported, malformed, or CPU-only responses (roadmap Task 1.1). Report the parsed feature sets and any mismatch with what modelctl expects. $ARGUMENTS
